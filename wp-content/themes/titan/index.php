...
if( $wp_query->is_posts_page ) {
	...
	$sidebar = titan_set( $meta, 'sidebar', 'default-sidebar' );
	$title = titan_set($meta1, 'header_title');
	$bg = titan_set($meta1, 'header_img');
} else {
	$settings  = _WSH()->option(); 
	if(titan_set($_GET, 'layout_style')) $layout = titan_set($_GET, 'layout_style'); else
	$layout = titan_set( $settings, 'archive_page_layout', 'right' );
	$sidebar = titan_set( $settings, 'archive_page_sidebar', 'default-sidebar' );
	$title = titan_set($settings, 'archive_page_header_title');
	$bg = titan_set($settings, 'archive_page_header_img');
}
$layout = titan_set( $_GET, 'layout' ) ? titan_set( $_GET, 'layout' ) : $layout;
...
<section class="breadcrumb-area" <?php if($bg):?>style="background-image:url('<?php echo esc_url($bg)?>');"<?php endif;?>>
	<div class="container text-center">
		<h1><?php if($title) echo wp_kses_post($title); else esc_html_e('Blog', 'titan'); ?></h1>
	</div>
</section>
<!--End breadcrumb area-->
