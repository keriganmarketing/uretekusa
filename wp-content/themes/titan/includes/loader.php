...
add_action( 'init', 'titan_bunch_theme_init');


if( !function_exists( 'titan_set' ) ) {
	function titan_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('titan_printr' ) ) {
	function titan_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('titan__font_awesome' ) ) {
	function titan_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = titan_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('titan_load_class' ) ) {
	function titan_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
titan_load_class( 'enqueue', 'helpers', false );
titan_load_class( 'bootstrap_walker', 'helpers', false );


if( titan_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function titan_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include TITAN_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include TITAN_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = TITAN_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = TITAN_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( titan_set( $_GET, 'page' ) == 'titan'.'_option' ) 
		{
			if( titan_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'titan_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = TITAN_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'titan'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'titan'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'titan' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'titan' ), // menu label
	));
