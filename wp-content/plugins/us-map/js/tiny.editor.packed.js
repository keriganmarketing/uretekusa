...
TINY.editor = function() {
    ...
    c['image'] = [20, 'Insert Image', 'i', 'insertimage',
        'Enter Image URL:', 'http://'
    ];
    ...
    c['blockquote'] = [26, 'blockquote', 'a', 'blockquote'];
    ...
    function edit(n, obj) {
        ...
        for (i; i < l; i++) {
            ...
            if (id == 'n') {
                ...
                div.title = x[1];
                ex = func == 'a' ? '.action("' + x[3] + '", 0, ' + (x[4] ||
                        0) + ')' : '.insert("' + x[4] + '", "' + x[5] +
                    '", "' + x[3] + '")';
                div.onmousedown = new Function(this.n + (id == 'print' ?
                    '.print()' : ex));
                div.onmouseover = new Function(this.n + '.hover(this, ' +
                    pos + ', 1)');
                div.onmouseout = new Function(this.n + '.hover(this, ' +
                    pos + ', 0)');
                h.appendChild(div);
                if (this.ie) {
                    div.unselectable = 'on'
    ...
    edit.prototype.hover = function(div, pos, dir) {
        this.getSelection();
        div.style.backgroundPosition = (dir ? '34px ' : '0px ') + (pos) +
            'px'
    };
        ...
        if (!this.d) {
            ...
            if (this.xhtml) {
                ...
                v = v.replace(/<b\b[^>]*>(.*?)<\/b[^>]*>/gi,
                    '<strong>$1</strong>');
                v = v.replace(/<i\b[^>]*>(.*?)<\/i[^>]*>/gi,
                    '<em>$1</em>');
                v = v.replace(/<u\b[^>]*>(.*?)<\/u[^>]*>/gi,
                    '<span style="text-decoration:underline">$1</span>'
                );
                v = v.replace(
                    /<(b|strong|em|i|u|blockquote) style="font-weight:normal;?">(.*)<\/(b|strong|em|i|u|blockquote)>/gi,
                    '$2');
                v = v.replace(
                    /<(b|strong|em|i|u|blockquote) style="(.*)">(.*)<\/(b|strong|em|i|u|blockquote)>/gi,
...
TINY.cursor = function() {
    return {
        top: function(e) {
            return T$$$() ? window.event.clientY + document.documentElement
                .scrollTop + document.body.scrollTop : e.clientY +
                window.scrollY
        }
    }
}();