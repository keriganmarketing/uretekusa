    ...
    <table class="table">
        ...
        foreach($res as $stat){
            ...
            <tr>
                ...
                <td><?php echo $stat->uid>0?"<a target='_blank' href='user-edit.php?user_id={$stat->uid}'>".get_user_by('ID', $stat->uid)->display_name."</a> (<a target='_blank' href='http://ip-api.com/#{$stat->ip}'>{$stat->ip}</a>)":"<a target='_blank' href='http://ip-api.com/#{$stat->ip}'>{$stat->ip}</a>"; ?></td>
            </tr>
            <?php
        }
        ?>
