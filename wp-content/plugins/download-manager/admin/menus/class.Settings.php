...
class Settings
{
    ...
    function initiateSettings()
    {
        ...
        $tabs['social-connects'] = array('id' => 'social-connects','icon'=>'fa fa-twitter', 'link' => 'edit.php?post_type=wpdmpro&page=settings&tab=social-connects', 'title' => 'Social Settings', 'callback' => array($this, 'socialConnects'));
    ...
    function pluginUpdate(){
        ...
        if(isset($_POST['__wpdm_suname']) && $_POST['__wpdm_suname'] != ''){
            ...
            delete_option('__wpdm_core_update_check');
            unset($_SESSION['__wpdm_download_url']);
            $purchased_items = remote_get('https://www.wpdownloadmanager.com/?wpdmppaction=getpurchaseditems&user=' . get_option('__wpdm_suname') . '&pass=' . urlencode(get_option('__wpdm_supass')));
        ...
        if(get_option('__wpdm_suname') != '') {
            ...
            if(!$purchased_items || wpdm_query_var('newpurchase') != '' ) {
                $purchased_items = remote_get('https://www.wpdownloadmanager.com/?wpdmppaction=getpurchaseditems&user=' . get_option('__wpdm_suname') . '&pass=' . urlencode(get_option('__wpdm_supass')));
                dd('https://www.wpdownloadmanager.com/?wpdmppaction=getpurchaseditems&user=' . get_option('__wpdm_suname') . '&pass=' . urlencode(get_option('__wpdm_supass')));
                update_option('__wpdm_purchased_items', $purchased_items);
            }
