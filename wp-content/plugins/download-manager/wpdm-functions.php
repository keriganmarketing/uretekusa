    ...
    if (!isset($package['quota']) || (isset($package['quota']) && $package['quota'] > 0 && $package['quota'] > $package['download_count']) || $package['quota'] == 0) {
        ...
        if ($lock === 'locked') {
            ...
            if ($embed == 1)
                ...
                $dataattrs = $popstyle == 'pop-over'? 'data-title="<button type=button id=\'close\' class=\'btn btn-danger btn-xs pull-right po-close\' style=\'margin-top:-1px;margin-right:-5px\'><i class=\'fa fa-times\'></i></button> '.__('Download','wpdmpro').' ' . $package['title'] . '"' : 'data-toggle="modal" data-target="#pkg_' . $package['ID'] . "_" . $unqid . '"';
        ...
        if(isset($package['terms_lock']) && $package['terms_lock'] != 0 && (!function_exists('wpdmpp_effective_price') || wpdmpp_effective_price($package['ID']) ==0)){
            ...
            $data = '<a href="#" data-toggle="modal" data-target="#termslockmodal" class="wpdm-terms-modal [btnclass]">
                                                  '.$package['link_label'].'
                                                </a>                                               
...
function wpdm_getlink()
{
    ...
    $exp = array('use' => get_option('__wpdm_private_link_usage_limit',3), 'expire' => 999999);
    ...
    if(isset($_POST['reCaptchaVerify'])){
        ...
        $ret = json_decode($ret);
        if($ret->success == 1){
            $download_url  = \WPDM\Package::expirableDownloadLink($id, $limit);
    ...
    if (isset($_POST['verify']) && $_POST['verify'] == 'email' && $file['email_lock'] == 1) {
        if (wpdm_verify_email($_POST['email'])) {
            ...
            $idle = isset($file['email_lock_idl_email']) ? $file['email_lock_idl_email'] : 0;
            if($idl == 0 || $idle == 0) {

                \WPDM\Email::send("email-lock", array('to_email' => $_POST['email'], 'package_name' => $file['post_title'], 'download_url' => $download_url));

            }
            if ($idl != 1) {
                $data['downloadurl'] = "";
                $data['error'] = '<div class="alert alert-success" data-title="'.__('DONE', 'wpdmpro').'">' . __('Download link sent to your email!', 'wpdmpro') . '</div>';
            } else {
                $data['downloadurl'] = $download_url;
                $data['error'] = '';
                if($idle == 0)
                    $data['msg'] = '<div class="alert alert-success" data-title="'.__('DONE', 'wpdmpro').'">' . __('Download link also sent to your email!', 'wpdmpro') . '</div>';
                else
                    $data['msg'] = '<div class="alert alert-success" data-title="'.__('DONE', 'wpdmpro').'">' . __('Download will be started shortly!', 'wpdmpro') . '</div>';
            }
            ...
            $data['downloadurl'] = "";
            $data['error'] = '<div class="alert alert-danger" data-title="ERROR">' . __('Invalid Email Address!', 'wpdmpro') . '</i>';
    ...
    if ($plock == 1 && $password != $file['password'] && !strpos("__" . $file['password'], "[$password]")) {
        $data['error'] = __('Wrong Password!','wpdmpro')." &nbsp; <span><i class='fa fa-repeat'></i> ".__('Try Again','wpdmpro')." </span>";
        $file = array();
    }
    if ($plock == 1 && $password == '') {
        $data['error'] = __('Wrong Password!', 'wpdmpro')." &nbsp; <span class='color-blue'><i class='fa fa-repeat'></i> ".__('Try Again','wpdmpro')." </span>";
        $file = array();
    }
    $ux = "";
    if ($plock == 1 && isset($file['ID'])) {
        update_post_meta($file['ID'], "__wpdmkey_" .$key, apply_filters('wpdm_download_link_expiration_limit', 3, $file));
    }
...
function wpdm_page_links($urltemplate, $total, $page = 1, $items_per_page = 10, $next_label = ' <i class="fa fa-long-arrow-right"></i> ', $prev_label = ' <i class="fa fa-long-arrow-left"></i> ')
{
    ...
    $pag->items($total);
    $pag->nextLabel($next_label);
    $pag->prevLabel($prev_label);
...
function wpdm_media_field($data){
    ...
    <div class="input-group">
        <input placeholder="<?php echo $data['placeholder']; ?>" type="url" name="<?php echo $data['name']; ?>" id="<?php echo isset($data['id'])?$data['id']:($id = uniqid()); ?>" class="form-control" value="<?php echo isset($data['value']) ? $data['value'] : ''; ?>"/>
        <span class="input-group-btn">
                        <button class="btn btn-default btn-media-upload" type="button" rel="#<?php echo isset($data['id'])?$data['id']:$id; ?>"><i class="fa fa-picture-o"></i></button>
                    </span>
    </div>
    <?php
    return ob_get_clean();
...
function wpdm_ask_for_custom_data($pid)
{
    ...
    <div class="form-group">
               <label><input type="checkbox" name="file[custom_form_field][name]" value="1" <?php if (isset($cff['name']) && $cff['name'] == 1) echo 'checked=checked'; ?> > <?php _e("Ask for Visitor's Name","wpdmpro");?></label> <br/>
                ...
                <?php echo __('After submitting form:','wpdmpro'); ?>
                <label><input type="radio" id="idl1" name="file[email_lock_idl]"
                              value="0" <?php if ($idl != 1) echo 'checked=checked'; ?>> <?php echo __('Mail Download Link','wpdmpro'); ?></label>
                <label id="email_lock_idl_lbl" <?php if ($idl == 1) echo 'class="dnedl"'; ?>><input type="radio" id="idl" name="file[email_lock_idl]"
                ...
                <label id="email_lock_idl_email_lbl"> & <input type="checkbox" id="idle" name="file[email_lock_idl_email]"
                      value="1" <?php if ($idle == 1) echo 'checked=checked'; ?> > <?php echo __('Do Not Mail Download Link','wpdmpro'); ?></label>
...
function wpdm_render_custom_data($html = '',  $pid)
{
    ...
    $labels['name_placeholder'] = __('Enter Your Name',"wpdmpro");
    if (!$cff) return '';

    foreach ($cff as $name => $value) {
        $html .= <<<DATA
        <div class="form-group">
    <label><nobr>{$labels[$name]}:</nobr></label><input placeholder="{$labels[$name.'_placeholder']}" type="text" name="custom_form_field[$name]" class="form-control email-lock-name" />
    </div>
DATA;
    }
    return $html;
...
function wpdm_archive_page_template($content){
    ...
    if(get_post_type()=='wpdmpro'){
        ...
        if(isset($cpageinfo['version'])) {
            $version = get_package_data($id, 'version', '1.0.0');
            $info .= "<li><i class='fa fa-history'></i> &nbsp;" . __('Version', 'wpdmpro') . " {$version}</li>";
        }

        if(isset($cpageinfo['view_count'])) {
            $view_count = get_package_data($id, 'view_count', 0);
            $info .= "<li><i class='fa fa-eye'></i>  &nbsp;{$view_count} " . __('View(s)', 'wpdmpro') . "</li>";
        }

        if(isset($cpageinfo['download_count'])) {
            $download_count = get_package_data($id, 'download_count', 0);
            $info .= "<li><i class='fa fa-download'></i>  &nbsp;{$download_count} " . __('Download(s)', "wpdmpro") . "</li>";
        }

        if(isset($cpageinfo['package_size'])) {
            $package_size = get_package_data($id, 'package_size');
            $info .= "<li><i class='fa fa-th'></i>  &nbsp;{$package_size}</li>";
        }

        if(isset($cpageinfo['download_link'])) {
            $download_link = \WPDM\Package::downloadLink($id);
            $info .= "<li>{$download_link}</li>";
        }
        ...
        if(get_option('__wpdm_cpage_style')=='ltpl'){
            return "<div class='w3eden'>".wpdm_fetch_template(get_option('__wpdm_cpage_template'), $id)."</div>";
        }
