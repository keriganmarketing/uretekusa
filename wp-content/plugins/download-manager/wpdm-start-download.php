...
if ($fileCount > 1 && !$idvdl) {
    ...
    if ($zipped == '' || !file_exists($zipped) || $cache_zip == 0) {
        ...
        $zip = new ZipArchive();
        else wp_die(__('Please activate "zlib" in your server', 'wpdmpro'));
        ...
        foreach ($files as $file) {
            ...
            if (file_exists(UPLOAD_DIR . $file))
                $zip->addFile(UPLOAD_DIR . $file, wpdm_basename($file));
    ...
    if (isset($_GET['ind'])) {
        $indfile = isset($files[esc_attr($_GET['ind'])])?$files[esc_attr($_GET['ind'])]:\WPDM_Crypt::Decrypt(esc_attr($_GET['ind']));
        //dd($indfile);
        /*
    ...
    if ($indfile != '' && file_exists(UPLOAD_DIR . $indfile))
        ...
        wpdm_download_data('file-not-found.txt', __('File not found or deleted from server', 'wpdmpro'));
        die();
    }
