...
;(function ($) {
  ...
  $.fn.ai1wm_modal = function () {
    // Add overlay to body
    $('<div />').addClass('ai1wmge-overlay').appendTo('body');

    // Remove visibility hidden
    $(this).removeClass('ai1wmge-not-visible').addClass('ai1wmge-visible');
...
var browse_folder = function browse_folder(folder, index) {
	v.items = false;
	v.folder = false;
	v.selectedItem = null;
	v.loading = true;
	...
	// Slice path items
	v.path = v.path.slice(0, index);

	// Get files and folders
	$.ajax({
		url: ai1wmge_settings.ajax.selector_url,
		type: 'GET',
		dataType: 'json',
...
jQuery(document).ready(function ($) {
	...
	if (ai1wmge_settings.token) {
		$.ajax({
			...
			if (!!result.message) {
				$('#ai1wmge-gdrive-details').html($('<div />').addClass('ai1wm-message ai1wm-error-message').append('<p>' + result.message + '</p>'));
	...
	$.ajax({
		url: ai1wmge_settings.ajax.folder_url,
		dataType: 'json',
		type: 'GET',
		dataFilter: function dataFilter(data, type) {
			return Ai1wm.Util.json(data);
		...
		var details = '<i class="ai1wmge-icon-gdrive"></i>';
		if (!!result.name) {
			details += '<span id="ai1wmge-gdrive-folder-name">' + result.name + '</span>';
		}

		if (!!result.id) {
			details += '<a href="https://drive.google.com/drive/u/0/folders/' + result.id + '" id="ai1wmge-gdrive-folder-open" target="_blank">[open]</a>';
		}

		$('#ai1wmge-gdrive-folder-details').html(details);
		$('#ai1wmge-gdrive-folder-id').val(result.id);
		$('#ai1wmge-gdrive-change').show();
	}).fail(function (xhr, status, error) {
		var result = $.parseJSON(xhr.responseText);
		if (!!result.message) {
			$('#ai1wmge-gdrive-folder-details').html($('<div />').addClass('ai1wm-message ai1wm-error-message').append('<p>' + result.message + '</p>'));
			$('#ai1wmge-gdrive-change').hide();
		}
	});

	// Get folder
	$('#ai1wmge-gdrive-change').click(function (e) {
		e.preventDefault();

		if (v === false) {
			v = new Vue({
				el: '#ai1wmge-settings-modal',
				...
				data: {
					...
					loading: true,
					selectedItem: null,
					preselectedItemID: $('#ai1wmge-gdrive-folder-id').val()
				},

				methods: {
					browse: function browse(item, index) {
						browse_folder(item, index);
					},
					...
					cancel: function cancel() {
						...
						v.selectedItem = null;
						v.loading = false;
						v.preselectedItemID = $('#ai1wmge-gdrive-folder-id').val();

						// Hide modal
						$('#ai1wmge-settings-modal').hide();

						// Hide overlay
						$('.ai1wmge-overlay').fadeOut();
					},

					select: function select(item) {
						v.selectedItem = item;
						v.preselectedItemID = null;
					},

					store: function store() {
						var item = v.selectedItem;
						v.preselectedItemID = item.id;

						// Set folder ID
						$('#ai1wmge-gdrive-folder-id').val(item.id);

						// Set folder name
						$('#ai1wmge-gdrive-folder-name').text(item.name);

						// Set folder URL
						$('#ai1wmge-gdrive-folder-open').attr('href', 'https://drive.google.com/drive/u/0/folders/' + item.id);

						// Hide modal
						$('#ai1wmge-settings-modal').hide();

						// Hide overlay
						$('.ai1wmge-overlay').hide();
					}
				}
			});
		} else {
		...
		// Show modal
		$('#ai1wmge-settings-modal').ai1wm_modal();

		// Browse folder
		browse_folder({ id: 'root', name: '<i class="ai1wmge-icon-gdrive"></i>', type: 'application/vnd.google-apps.folder' });
	});
});
