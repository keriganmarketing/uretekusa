...
var browse_folder = function browse_folder(folder, index) {
	...
	$.ajax({
		url: ai1wmge_import.ajax.browser_url,
		type: 'GET',
		dataType: 'json',
		...
		v.num_hidden_files = result.num_hidden_files;
		v.path.push(folder);
	}).fail(function (err) {
			...
			v = new Vue({
				...
				methods: {
					...
					restore: function restore(item) {
						...
						var options = Ai1wm.Util.form('#ai1wm-import-form').concat({ name: 'storage', value: storage }).concat({ name: 'archive', value: item.name }).concat({ name: 'file_id', value: item.id }).concat({ name: 'file_size', value: item.bytes }).concat({ name: 'gdrive', value: 1 });
